...
def load_data(filename,time_steps,features,split,time_range):
	...
	reframed_data = reframed_data.iloc[:,[0,1,time_range*features,time_range*features+1]]  
	print('the shape of reframed_data: ',reframed_data.shape)#(490,52)
	print(reframed_data.head(30))
...
# load all data
def load_data_all(files,time_steps,features,split,time_range):
	# get scaler
	reframed_data_all = np.empty((1,4))
	for f_index in range(len(files)):
		raw_data = pd.read_csv(files[f_index])
		raw_data = raw_data.iloc[:,[4,3]] # ,5,6]
		#print('the shape of raw_data: ',raw_data.shape)#(500,8)
		#print(raw_data.head(12))
		values = np.array(raw_data).astype(float)
		...
		reframed_data_all = concatenate((reframed_data_all, reframed_data),axis=0)

	reframed_data_all = reframed_data_all[1:,:] # because of empty
	print('the shape of reframed_data_all',reframed_data_all.shape)
	print(reframed_data_all)
	#normalize features  (second axis should be the same)
	scaler = MinMaxScaler(feature_range = (0,1)) #it will be better if split to several section
	scaled_data = scaler.fit_transform(reframed_data_all) #(,6)


	# get train data
	X_train_total = np.empty((1,20,2))
	Y_train_total = np.empty((1,2))
	X_test_total = np.empty((1,20,2))
	Y_test_total = np.empty((1,2))
	...
	for f_index in range(len(files)):
		...
		print(reframed_data)
		reframed_data = scaler.transform(reframed_data) 
		print(reframed_data)

		train_row = int(round(split * (reframed_data.shape[0])))
		train_data = reframed_data[:train_row,:]
		#np.random.shuffle(train_data)
		X_train, Y_train = train_data[:,:-2], train_data[:,-2:]
		X_test, Y_test = reframed_data[train_row:,:-2], reframed_data[train_row:,-2:]
		...
		X_train_total = concatenate((X_train_total,X_train_3D),axis=0)
		Y_train_total = concatenate((Y_train_total,Y_train),axis=0)
		X_test_total = concatenate((X_test_total,X_test_3D),axis=0)
		Y_test_total = concatenate((Y_test_total,Y_test),axis=0)
	...
	return X_train_total,Y_train_total,X_test_total,Y_test_total,scaler 
...
if __name__ == '__main__':
	...
	time_steps = 20
	forecast_steps = [5,10,15,20,25,30]#[1,5,10,15,20] real
	files = ['X_Y_dX_dY_11_13.csv', 'X_Y_dX_dY_11_18.csv', 'X_Y_dX_dY_11_22.csv', 'X_Y_dX_dY_11_27.csv']
	...
	for fore_step in forecast_steps:
		...
		features = 2
		split = 0.9
		X_train,Y_train,X_test,Y_test,scaler = load_data_all(files, time_steps, features, split , time_range)
		...
		epochs = 50
		batch_size = 30#int(round(X_train.shape[0]/200))
		model = build_train_model(X_train,Y_train,X_test,Y_test, epochs, batch_size)

		#save model  HDF5 file

		if fore_step == 5:
			model.save('./model/model_two_two_20_5.h5')
		if fore_step == 10:
			model.save('./model/model_two_two_20_10.h5')
		if fore_step == 15:
			model.save('./model/model_two_two_20_15.h5')
		if fore_step == 20:
			model.save('./model/model_two_two_20_20.h5')
		if fore_step == 25:
			model.save('./model/model_two_two_20_25.h5')
		if fore_step == 30:
			model.save('./model/model_two_two_20_30.h5')
		...
		#other dataset  0
		X_train,Y_train,X_test,Y_test,scaler = load_data('X_Y_dX_dY_11_13.csv', time_steps, features, 0.1, time_range)
