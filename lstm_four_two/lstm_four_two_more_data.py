...
def load_data(filename,time_steps,features,split,time_range):
	...
	if filename == 'X_Y_dX_dY_11_13.csv':
		raw_data_1 = pd.read_csv('X_Y_dX_dY_11_18.csv')
		raw_data_1 = raw_data_1.iloc[:,[4,3,5,6]]
		print(raw_data.shape)
		print(raw_data_1.shape)
		raw_data = concatenate((raw_data,raw_data_1),axis=0)
...
def build_train_model(X_train,Y_train,X_test,Y_test,epochs,batch_size):
	...
	model.add(LSTM(units=64,input_shape=(time_steps,n_features),return_sequences=True))
...
if __name__ == '__main__':
	...
	time_steps = 20
	forecast_steps = [10]#[0,5,10,15,20,25,30,35,40,45,50]
	...
	for fore_step in forecast_steps:
		...
		features = 4
		split = 0.05
		#X_Y_dX_dY_11_13.csv  X_Y_dX_dY_11_18.csv
		...
		'''
		my_model = load_model('my_model.h5') # model_four_two_0.1s.h5   from 0.5s to predict 0.1s
		...
		'''
		#other dataset  1
		X_train,Y_train,X_test,Y_test,scaler = load_data('X_Y_dX_dY_11_18.csv', time_steps, features, 0.15, time_range)
