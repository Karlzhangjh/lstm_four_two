...
def load_data(filename,time_steps,features,split,time_range):
	...
	if split == 0:	# all for test
		X_test, Y_test = scaled_data[:,:-2], scaled_data[:,-2:]
		X_test = X_test.reshape(X_test.shape[0],1,X_test.shape[1])
		X_test_3D = np.zeros((X_test.shape[0]-time_range-1, time_steps, features), dtype=np.float)#9 numbers to front
		Y_test = Y_test[:X_test.shape[0]-time_range-1, :]
		for i in range(X_test.shape[0]-time_range-1):
				for j in range(time_steps):
					X_test_3D[i,j,:] = X_test[i+j,0,:]

		X_train_3D = np.zeros((1,1))
		...
		X_train_3D = np.zeros((X_train.shape[0]-time_range-1, time_steps, features), dtype=np.float)#9 numbers to front
		X_test_3D = np.zeros((X_test.shape[0]-time_range-1, time_steps, features), dtype=np.float)#9 numbers to front
...
def build_train_model(X_train,Y_train,X_test,Y_test,epochs,batch_size):
	...
	model.add(LSTM(units=32,input_shape=(time_steps,n_features),return_sequences=True))
	...
	model.add(Dense(units=ahead,input_dim=64))
	#model.add(Dropout(0.2))
...
'''
def more_forward_predict(model,X_test):
	Y_hat = model.predict(X_test)
	for i in range(X_test.shape[0]):
		X_row = X_test[i,:] 
		for j in range(10):
			Y_temp = model.predict(X_row)
			if j == 9:
				Y_hat[i] = Y_temp
			X_row[:-1] = X_row[1:]
			X_row[-1] = Y_temp	
...
def plot_result(predict_result,true_result,figs,colors):
	fig_m = 4
	fig_n = 2
	...
	if figs == 0 :
		...
		plt.plot(predict_result[:,0],colors,label='Pred X')
		plt.legend(loc="upper left")
		# Y
		plt.subplot(fig_m,fig_n,3)
		if colors == 'r--':
			plt.plot(true_result[:,1],'k-',label='True Y')
		plt.plot(predict_result[:,1],colors,label='Pred Y')
...
if __name__ == '__main__':
	...
	time_steps = 20
	forecast_steps = [10]#[1,5,10,15,20]  # 1:next-step 5:5th-step
	colors = ['r--','b--','g--']
	...
	for fore_step in forecast_steps:
		...
		'''
		epochs = 50
		batch_size = 100#int(round(X_train.shape[0]/200))
		model = build_train_model(X_train,Y_train,X_test,Y_test, epochs, batch_size)
		
		#save model  HDF5 file
		if fore_step == 0:
			model.save('my_model_0.1s.h5')
		elif fore_step == 5:
			model.save('my_model_0.5s.h5')
		elif fore_step == 10:
			model.save('my_model_1s.h5')
		'''
		#load model
		my_model_sigle = load_model('my_model_0.1s.h5')
		my_model = load_model('my_model_1s.h5')
		...
		plot_result(predict_result,true_result,0,colors[0])
		predict_result,true_result = pos_predict(my_model_sigle,scaler,X_test,Y_test)
		plot_result(predict_result[10:,:],true_result[10:,:],0,colors[1])

		split = 0
		#other dataset  1
		X_train,Y_train,X_test,Y_test,scaler = load_data('X_Y_dX_dY_11_18.csv', time_steps, features, split, time_range)
		print('The shape of other dataset 1 --------------------',X_train.shape,Y_train.shape,X_test.shape,Y_test.shape)
		predict_result,true_result = pos_predict(my_model,scaler,X_test,Y_test)
		plot_result(predict_result,true_result,1,colors[0])
		predict_result,true_result = pos_predict(my_model_sigle,scaler,X_test,Y_test)
		plot_result(predict_result[10:,:],true_result[10:,:],1,colors[1])

		#other dataset  2
		X_train,Y_train,X_test,Y_test,scaler = load_data('X_Y_dX_dY_11_22.csv', time_steps, features, split, time_range)
		print('The shape of other dataset 2 --------------------',X_train.shape,Y_train.shape,X_test.shape,Y_test.shape)
		predict_result,true_result = pos_predict(my_model,scaler,X_test,Y_test)
		plot_result(predict_result,true_result,2,colors[0])
		predict_result,true_result = pos_predict(my_model_sigle,scaler,X_test,Y_test)
		plot_result(predict_result[10:,:],true_result[10:,:],2,colors[1])

		#other dataset  3
		X_train,Y_train,X_test,Y_test,scaler = load_data('X_Y_dX_dY_11_27.csv', time_steps, features, split, time_range)
		print('The shape of other dataset 3 --------------------',X_train.shape,Y_train.shape,X_test.shape,Y_test.shape)
		predict_result,true_result = pos_predict(my_model,scaler,X_test,Y_test)	
		plot_result(predict_result,true_result,3,colors[0])
		predict_result,true_result = pos_predict(my_model_sigle,scaler,X_test,Y_test)
		plot_result(predict_result[10:,:],true_result[10:,:],3,colors[1])
